...
//get average of each month with years combined
    ...
    // .rollup(function(v) { 
    //     return d.key.values.key.value.avg
    // })
    // .entries(data);
...
//create x domain based on a variable I created in my object, averaged year: It removes the day and year values in order to do a month-level analysis
    x.domain(d3.extent(data, function(d) { 
        d.values, function(d){ return d.values;}
        return d.avg}));
...
function renderB(data, i) {
    ...
    var maxTest = testMax.reduce(function(a, b) {
        return Math.max(a, b);
    });
...
////////////////Load Data//////////////////
////////////////////////////////////////
d3.csv("/thesis_versions/presentation1/data/malawi_landsat_5_7_8_1990-2018.csv").then(function(data) {
    ...
    sortDataByMonthMean.forEach(function(d, i){
    d.class = 'Vegetation'
        
         d.values.forEach(function(d){
            testMax.push(d.value.avg)
        });    
